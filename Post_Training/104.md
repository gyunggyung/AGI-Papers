...
그가 함께 공유한 분석글은 이 비전을 '에너지 기반 모델(EBM)'로 설명합니다. 단순히 그럴듯한 말을 뱉는(Likelihood Maximization) 게 아니라, 현실과 모순되는 상태(High Energy)를 거부하고 정합성을 찾아가는(Energy Minimization) 과정이 곧 지능이라는 것이죠.
...
지금 Maxime Labonne 님에게 자문을 구하고 LFM2-1.2B-KOEN-MT 모델의 약점을 줄이기 위해 추가 SFT 작업을 준비하고 있습니다. 해당 작업이 끝나면 바로 GRPO + DuPO 작업을 통해서 성능을 올려보려고 합니다. 실제로 해당 논문에서 말한 작업이 되는지 검증하고 다시 공유하겠습니다. (I plan to tune this way without DPO)
